...
`confirm_close` controls whether closing a workspace asks for confirmation. `prompt_new_tab_name` controls whether new tabs ask for a label first. `prompt_new_workspace_name` controls whether interactive TUI actions ask for an optional workspace label before creating it; CLI and API creation remain immediate.
