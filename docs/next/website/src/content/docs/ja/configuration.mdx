...
`confirm_close` はワークスペースを閉じるときに確認を求めるかどうかを制御します。`prompt_new_tab_name` は新しいタブで先にラベルを尋ねるかどうかを制御します。`prompt_new_workspace_name` は、対話的な TUI 操作でワークスペースを作成する前に任意のラベルを尋ねるかどうかを制御します。CLI と API からの作成は引き続き即時に行われます。
